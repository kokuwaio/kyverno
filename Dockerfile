...
FROM docker.io/library/bash:5.3.20@sha256:d0c3166d3b69a70c71a8c85a69a32d14395dab59fef1f63ee583aa91a935080b AS yq
...
FROM docker.io/library/bash:5.3.20@sha256:d0c3166d3b69a70c71a8c85a69a32d14395dab59fef1f63ee583aa91a935080b AS kustomize
SHELL ["/usr/local/bin/bash", "-u", "-e", "-c"]
ARG TARGETARCH
...
FROM docker.io/library/bash:5.3.20@sha256:d0c3166d3b69a70c71a8c85a69a32d14395dab59fef1f63ee583aa91a935080b AS kyverno
SHELL ["/usr/local/bin/bash", "-u", "-e", "-c"]
ARG TARGETARCH
RUN [[ $TARGETARCH == amd64 ]] && export ARCH=x86_64; \
	[[ $TARGETARCH == arm64 ]] && export ARCH=arm64; \
	[[ -z ${ARCH:-} ]] && echo "Unknown arch: $TARGETARCH" && exit 1; \
	wget -q "https://github.com/kyverno/kyverno/releases/download/v1.19.1/kyverno-cli_v1.19.1_linux_$ARCH.tar.gz" --output-document=- | \
