...
FROM docker.io/library/bash:5.3.9@sha256:80e26d49514c8660876a7db371ee93231a336716a1fd8e9908c8fb7fad88f270 AS yq
SHELL ["/usr/local/bin/bash", "-u", "-e", "-c"]
ARG TARGETARCH
RUN wget -q "https://github.com/mikefarah/yq/releases/download/v4.52.5/yq_linux_$TARGETARCH.tar.gz" --output-document=- | \
	tar --gz --extract --to-stdout "./yq_linux_$TARGETARCH" > /usr/local/bin/yq && chmod 555 /usr/local/bin/yq

FROM docker.io/library/bash:5.3.9@sha256:80e26d49514c8660876a7db371ee93231a336716a1fd8e9908c8fb7fad88f270 AS kustomize
SHELL ["/usr/local/bin/bash", "-u", "-e", "-c"]
ARG TARGETARCH
...
FROM docker.io/library/bash:5.3.9@sha256:80e26d49514c8660876a7db371ee93231a336716a1fd8e9908c8fb7fad88f270 AS kyverno
SHELL ["/usr/local/bin/bash", "-u", "-e", "-c"]
ARG TARGETARCH
RUN [[ $TARGETARCH == amd64 ]] && export ARCH=x86_64; \
	[[ $TARGETARCH == arm64 ]] && export ARCH=arm64; \
	[[ -z ${ARCH:-} ]] && echo "Unknown arch: $TARGETARCH" && exit 1; \
	wget -q "https://github.com/kyverno/kyverno/releases/download/v1.17.1/kyverno-cli_v1.17.1_linux_$ARCH.tar.gz" --output-document=- | \
