...
FROM docker.io/library/bash:5.3.15@sha256:66b753fd64a652fdaf62121cb4fdbdcbe3c80810f00c33b3b175fde8d0272802 AS yq
SHELL ["/usr/local/bin/bash", "-u", "-e", "-c"]
ARG TARGETARCH
RUN wget -q "https://github.com/mikefarah/yq/releases/download/v4.53.3/yq_linux_$TARGETARCH.tar.gz" --output-document=- | \
	tar --gz --extract --to-stdout "./yq_linux_$TARGETARCH" > /usr/local/bin/yq && chmod 555 /usr/local/bin/yq

FROM docker.io/library/bash:5.3.15@sha256:66b753fd64a652fdaf62121cb4fdbdcbe3c80810f00c33b3b175fde8d0272802 AS kustomize
SHELL ["/usr/local/bin/bash", "-u", "-e", "-c"]
ARG TARGETARCH
...
FROM docker.io/library/bash:5.3.15@sha256:66b753fd64a652fdaf62121cb4fdbdcbe3c80810f00c33b3b175fde8d0272802 AS kyverno
SHELL ["/usr/local/bin/bash", "-u", "-e", "-c"]
ARG TARGETARCH
RUN [[ $TARGETARCH == amd64 ]] && export ARCH=x86_64; \
	[[ $TARGETARCH == arm64 ]] && export ARCH=arm64; \
	[[ -z ${ARCH:-} ]] && echo "Unknown arch: $TARGETARCH" && exit 1; \
	wget -q "https://github.com/kyverno/kyverno/releases/download/v1.18.1/kyverno-cli_v1.18.1_linux_$ARCH.tar.gz" --output-document=- | \
