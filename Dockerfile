...
FROM docker.io/library/bash:5.3.9@sha256:32ca1653c68dd414bbdcdb9d5dc811010022b04e37bfcc886a5d052fc0c56656 AS yq
...
FROM docker.io/library/bash:5.3.9@sha256:32ca1653c68dd414bbdcdb9d5dc811010022b04e37bfcc886a5d052fc0c56656 AS kustomize
SHELL ["/usr/local/bin/bash", "-u", "-e", "-c"]
ARG TARGETARCH
...
FROM docker.io/library/bash:5.3.9@sha256:32ca1653c68dd414bbdcdb9d5dc811010022b04e37bfcc886a5d052fc0c56656 AS kyverno
SHELL ["/usr/local/bin/bash", "-u", "-e", "-c"]
ARG TARGETARCH
RUN [[ $TARGETARCH == amd64 ]] && export ARCH=x86_64; \
	[[ $TARGETARCH == arm64 ]] && export ARCH=arm64; \
	[[ -z ${ARCH:-} ]] && echo "Unknown arch: $TARGETARCH" && exit 1; \
	wget -q "https://github.com/kyverno/kyverno/releases/download/v1.16.2/kyverno-cli_v1.16.2_linux_$ARCH.tar.gz" --output-document=- | \
