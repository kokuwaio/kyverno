...
ARG TARGETARCH
RUN wget -q "https://github.com/mikefarah/yq/releases/download/v4.52.4/yq_linux_$TARGETARCH.tar.gz" --output-document=- | \
...
RUN [[ $TARGETARCH == amd64 ]] && export ARCH=x86_64; \
	...
	wget -q "https://github.com/kyverno/kyverno/releases/download/v1.17.0/kyverno-cli_v1.17.0_linux_$ARCH.tar.gz" --output-document=- | \
